...
namespace :heroku do
  desc "Load the config (config/travis.yml) into Heroku based on ENV['remote']"
  task :config do
    remote = ENV['REMOTE']
  ...
  namespace :staging do
    desc "Load the staging config (config/travis.yml) into Heroku"
    task :config do
      ENV['REMOTE'] = 'staging'
  ...
  namespace :production do
    desc "Load the production config (config/travis.yml) into Heroku"
    task :config do
